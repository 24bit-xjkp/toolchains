...
def build() -> None:
    ...
    basic_option = ("--disable-werror", "--enable-nls", f"--target={env.target}", f"--prefix={env.prefix}")
    gcc_option = basic_option + ("--enable-multilib", "--enable-languages=c,c++", "--disable-sjlj-exceptions", "--enable-threads=win32")
    mingw_option = (f"--host={env.target}", f"--prefix={os.path.join(env.prefix, env.target)}", "--with-default-msvcrt=ucrt")
    # 编译binutils
